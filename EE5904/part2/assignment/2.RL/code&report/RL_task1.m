...
for Dis_factor = [0.5, 0.9]
    for Lr_type = [1, 2, 3, 4]
        goal_reached_num = 0;
        Execution_times = [];
        for run = 1:10
            tic;
            Q_values = Q_learning(reward, Dis_factor, N, initial_S, end_S, Lr_type, Lr_min, Q_threshold);
            toctime = toc;
            [path, R_t] = get_trajectory(Q_values, reward, Dis_factor);
